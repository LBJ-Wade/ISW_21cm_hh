	...
	def c_z (self):
		...
		for i in range(self.number_of_k2):
			...
			for j in range(len(new_z)-1):
				...
				dCdz = 1/hubble[j] * (-(1+new_z[j])*2/3*b_dot[j] + Tr[j]/Tm[j]*gamma[j]*C/(1+new_z[j])) + C/(1+new_z[j])
				C -= dCdz*dz
	...
	def cl21T (self, z_m, w):
		...
		for l in self.l_list:
			...
			for j in range(len(kk)):
				...
				transfer_21.append (T*bb+T_2*(1+z[j]))
